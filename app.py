...
for k,v in {
    ...
    'history':[],'participant_no':1,'api_error':None,'source':None,'show_answer':False,
...
if st.query_params.get('aura_timeout')=='1':
    ...
    if st.session_state.stage=='challenge':
        for k,v in {'stage':'welcome','language':None,'question':None,'start_time':None,'elapsed':None,'api_error':None,'source':None,'show_answer':False}.items():
            st.session_state[k]=v
    st.rerun()
...
def start(lang):
    ...
    st.session_state.question=q; st.session_state.history.append(qhash(q)); st.session_state.start_time=time.perf_counter(); st.session_state.elapsed=None; st.session_state.show_answer=False; st.session_state.stage='challenge'; st.rerun()
def review():
    st.session_state.elapsed=time.perf_counter()-st.session_state.start_time
    st.session_state.show_answer=False   # timer stops here; answer stays hidden until revealed
...
def nxt():
    ...
    for k,v in {'stage':'welcome','language':None,'question':None,'start_time':None,'elapsed':None,'api_error':None,'source':None,'show_answer':False}.items(): st.session_state[k]=v
    st.rerun()
...
def review_side_html(final_time, q, show_answer=True):
    """Right-column panel for the review stage: time-taken ring + (optionally) the Correct Output box below it.
    When show_answer is False the answer stays hidden until the in-charge reveals it."""
    if show_answer:
        ans = (f'<div class="answer"><span class="alab">Correct Output</span>'
               f'<span class="aval">{_html.escape(str(q["answer"]))}</span>'
               f'<span class="aexp">{_html.escape(q["explanation"])}</span></div>')
        cap = 'Time you took before pressing Review Answer.'
    else:
        ans = ''
        cap = 'Timer stopped. Press “Show Answer” to reveal the correct output.'
    body = (
...
if st.session_state.stage=='welcome':
    ...
    with col_r:
        if not st.session_state.show_answer:
            # Timer already stopped. Keep the answer hidden until the in-charge presses Show Answer.
            components.html(review_side_html(fmt(st.session_state.elapsed), q, show_answer=False),
                            height=max(340, min(left_h, 420)), scrolling=False)
            st.markdown('<div class="timer-col-btn"></div>', unsafe_allow_html=True)
            ...
            components.html(review_side_html(fmt(st.session_state.elapsed), q, show_answer=True),
                            height=right_h, scrolling=False)
